...
router.route('/')
  ...
  .post((req, res) => {
    const filePath = path.join(__dirname, '../db/usuarios.json');
    const { nome, email, user, pwd, level, status } = req.body;
...
router.route('/:id')
  ...
  .put((req, res) => {
    ...
    fs.readFile(filePath, 'utf8', (err, data) => {
      ...
      usuarios[index] = {
        ...usuarios[index],
        name: nome || usuarios[index].name,
        email: email || usuarios[index].email,
        user: user || usuarios[index].user,
        pwd: pwd || usuarios[index].pwd,
        level: level || usuarios[index].level,
